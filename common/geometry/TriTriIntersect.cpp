...
bool IntersectTriTri(const MathLib::Vector3 &v0, const MathLib::Vector3 &v1, const MathLib::Vector3 &v2, const MathLib::Vector3 &u0, const MathLib::Vector3 &u1, const MathLib::Vector3 &u2)
{
	...
	for (int d=0; d<3; d++) {
		V0[d] = v0[d]; V1[d] = v1[d]; V2[d] = v2[d];
		U0[d] = u0[d]; U1[d] = u1[d]; U2[d] = u2[d];
	}
...
{
	double deg = Acos(n.dot(m));
	if (opp&&deg<180-ae || !opp&&deg>ae) {
		return false;
	}
	double pd0 = n.dot(u0-v0);
	double pd1 = n.dot(u1-v0);
	double pd2 = n.dot(u2-v0);
	double dist = Max(Abs(pd0),Max(Abs(pd1),Abs(pd2)));
	if (dist > de) {
		return false;
	}
	Matrix4d tm;
	MathLib::Vector2 tv[3], tu[3];
	tm.settranslate(-u0[0],-u0[1],-u0[2]).rotate(Acos(m.dot(ML_AZ)),m.cross(ML_AZ));
	tv[0] = tm.transform(v0);
	tv[1] = tm.transform(v1);
	tv[2] = tm.transform(v2);
	tu[0] = ML_O;
...
bool ContactTriTri(const MathLib::Vector3 &v0, const MathLib::Vector3 &v1, const MathLib::Vector3 &v2, const MathLib::Vector3 &n,
				   const MathLib::Vector3 &u0, const MathLib::Vector3 &u1, const MathLib::Vector3 &u2, const MathLib::Vector3 &m, float de)
{
	double pd0 = n.dot(u0-v0);
	double pd1 = n.dot(u1-v0);
	double pd2 = n.dot(u2-v0);
...
//////////////////////////////////////////////////////////////////////////
// 2D
int which_side_2d(const MathLib::Vector2 pl[3], const MathLib::Vector2 &p, const MathLib::Vector2 d)
{
	...
	return (positive > 0 ? 1 : -1);	// note that in the case when pos. and neg. are both nonzero it has already returned
	// return (zero == 0 ? (positive > 0 ? 1 : -1) : 0); // if you see line/point on as intersected too
}
...
bool IntersectTriTri2D(const MathLib::Vector2 t0[3], const MathLib::Vector2 t1[3])
{
	...
	for (i0 = 0, i1 = 2; i0 < 3; i1 = i0++)
	{
		...
		dir.x = t0[i0].y - t0[i1].y;
		dir.y = t0[i1].x - t0[i0].x;
		if (which_side_2d(t1, t0[i1], dir) > 0)
		{
	...
	for (i0 = 0, i1 = 2; i0 < 3; i1 = i0++)
	{
		...
		dir.x = t1[i0].y - t1[i1].y;
		dir.y = t1[i1].x - t1[i0].x;
		if (which_side_2d(t0, t1[i1], dir) > 0)
		{
